...
typedef struct LocalDir LocalDir;


/* Dirent functions */
static LocalDir *opendir(const char *dirname);
static _WDIR *_wopendir(const wchar_t *dirname);

static struct dirent *readdir(LocalDir *dirp);
static struct _wdirent *_wreaddir(_WDIR *dirp);

static int readdir_r(
	LocalDir *dirp, struct dirent *entry, struct dirent **result);
static int _wreaddir_r(
	_WDIR *dirp, struct _wdirent *entry, struct _wdirent **result);

static int closedir(LocalDir *dirp);
static int _wclosedir(_WDIR *dirp);

static void rewinddir(LocalDir *dirp);
static void _wrewinddir(_WDIR *dirp);

static long telldir(LocalDir *dirp);
static long _wtelldir(_WDIR *dirp);

static void seekdir(LocalDir *dirp, long loc);
...
static LocalDir *opendir(const char *dirname)
{
	...
	struct LocalDir *dirp = (LocalDir*) malloc(sizeof(struct LocalDir));
	if (!dirp)
		return NULL;
...
/* Seek directory stream to offset */
static void
seekdir(LocalDir *dirp, long loc)
{
	if (!dirp)
		return;
