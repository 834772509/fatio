...
union printf_arg
{
	...
	enum
	{
		...
		PRINTF_LONG,
		PRINTF_LONGLONG,
		PRINTF_UNSIGNED_INT = 3,
		PRINTF_UNSIGNED_LONG,
		PRINTF_UNSIGNED_LONGLONG,
		PRINTF_STRING,
		PRINTF_GUID
	} type;
	long long ll;
...
static char grub_debug_flags[256] = "enum";
	...
	const char* fmt, ...)
{
	...
	{
		grub_snprintf(debug_title, 256, "%s:%d:%s", file, line, condition);
		va_start(args, fmt);
		grub_vsnprintf(debug_info, 512, fmt, args);
		va_end(args);
		MessageBoxA(NULL, debug_info, debug_title, MB_OK);
	}
}
...
unsigned long
grub_strtoul(const char* str, const char** const end,
	int base)
{
...
unsigned long long
grub_strtoull(const char* str, const char** const end,
	int base)
{
	...
	int fmt_check, grub_size_t max_args)
{
	...
	while ((c = *fmt++) != 0)
	{
		...
		case 'o':
		case 'u':
			args->ptr[curn].type = PRINTF_UNSIGNED_INT + longfmt;
			break;
		case 'd':
			args->ptr[curn].type = PRINTF_INT + longfmt;
			break;
		case 'p':
			if (sizeof(void*) == sizeof(long long))
				args->ptr[curn].type = PRINTF_UNSIGNED_LONGLONG;
			else
				args->ptr[curn].type = PRINTF_UNSIGNED_INT;
			if (*(fmt) == 'G') {
				args->ptr[curn].type = PRINTF_GUID;
				++fmt;
			}
			break;
		case 's':
			args->ptr[curn].type = PRINTF_STRING;
			break;
		case 'C':
		case 'c':
			args->ptr[curn].type = PRINTF_INT;
			break;
		}
	}
...
parse_printf_args(const char* fmt0, struct printf_args* args, va_list args_in)
{
	...
	for (n = 0; n < args->count; n++)
		...
		case PRINTF_LONG:
			...
			break;
		case PRINTF_UNSIGNED_INT:
			args->ptr[n].ll = va_arg(args_in, unsigned int);
			break;
		case PRINTF_UNSIGNED_LONG:
			args->ptr[n].ll = va_arg(args_in, unsigned long);
			break;
		case PRINTF_LONGLONG:
		case PRINTF_UNSIGNED_LONGLONG:
...
grub_fatal(const char* fmt, ...)
{
	...
	va_start(ap, fmt);
	grub_vsnprintf(buf, 256, fmt, ap);
	va_end(ap);

	MessageBoxA(NULL, buf, "GRUB Fatal Error", MB_OK | MB_ICONERROR);
	exit(-1);
}
...
grub_utf8_to_utf16_alloc(const char* str8, grub_uint16_t** utf16_msg, grub_uint16_t** last_position)
{
	...
	{
		grub_error(GRUB_ERR_BAD_ARGUMENT, "string too long");
		*utf16_msg = NULL;
		return -1;
	}
