...
iterate_disk(const char* disk_name, void* data)
{
	...
	ctx->ents = NULL;
	ctx->tail = NULL;
	(void)grub_partition_iterate(dev, iterate_partition, ctx);
	grub_disk_close(dev);
...
int
grub_disk_iterate(grub_disk_dev_iterate_hook_t hook, void* hook_data)
{
	struct grub_disk_iterate_ctx ctx = { hook, hook_data, NULL, NULL };
