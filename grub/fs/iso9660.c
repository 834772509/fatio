...
static grub_err_t
grub_iso9660_dir(grub_disk_t disk, const char* path,
...
grub_iso9660_open(struct grub_file* file, const char* name)
{
	...
	data = grub_iso9660_mount(file->disk);
	if (!data)
		goto fail;
...
static grub_err_t
grub_iso9660_label(grub_disk_t disk, char** label)
{
	struct grub_iso9660_data* data;
	data = grub_iso9660_mount(disk);
...
static grub_err_t
grub_iso9660_uuid(grub_disk_t disk, char** uuid)
{
	struct grub_iso9660_data* data;
...
static grub_err_t
grub_iso9660_mtime(grub_disk_t disk, grub_int64_t* timebuf)
{
	struct grub_iso9660_data* data;
	grub_err_t err;
